...
interface RewardBreakdownProps {
  mode: 'free' | 'paid';
  /** Token used to pay the game fee. Defaults to 'CELO'. */
  currency?: 'CELO' | 'cUSD';
  wagerAmount?: number;
  txHash?: string;
}
...
/** Currency badge shown next to the fee label. */
function CurrencyBadge({ currency }: { currency: 'CELO' | 'cUSD' }) {
  const isCusd = currency === 'cUSD';
  return (
    <span
...
export default function RewardBreakdown({
  ...
  const wagerLabel = wagerAmount && wagerAmount > 0
    ? `${wagerAmount.toFixed(4)} ${currency}`
    : null;
  ...
  return (
    ...
    >
      <div className="flex items-start justify-between gap-2">
        <div>
          <p className="font-mono text-xs text-[var(--color-text-secondary)] uppercase tracking-widest mb-0.5">
            Fee Distribution
          </p>
      ...
      {txHash && (
        <a
          ...
          className="inline-flex items-center gap-1.5 font-sans text-xs text-primary hover:text-primary-dark underline underline-offset-2 transition-colors w-fit"
        >
          View on Blockscout
          <svg
            className="w-3 h-3"
      ...
      <p className="font-sans text-xs text-[var(--color-text-secondary)]">
        Royalties distributed automatically on-chain via Celo.
      </p>
    </motion.div>
  );
}
