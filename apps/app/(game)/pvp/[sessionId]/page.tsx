...
import GameEngine, { type Question, type QuestionResult } from '../../../../components/game/GameEngine';
...
export default function PvpSessionPage() {
  ...
  const [phase, setPhase] = useState<'waiting' | 'playing' | 'submitted' | 'done'>('waiting');
  ...
  useEffect(() => {
    ...
    const amount = isTie ? -650 : isWinner ? 3700 : -5000;
    addEntry({
      mode: 'competitive',
  ...
  useEffect(() => {
    if (session.status >= 1 && session.questionIds.length > 0 && questions.length === 0 && !loadingQ) {
      setLoadingQ(true);
  ...
  const handleComplete = useCallback(async (results: QuestionResult[]) => {
    setPhase('submitted');
  ...
  }, [address, sessionId, addEntry]);

  if (!isReady) return <div className="flex-1 flex items-center justify-center"><Spinner size="lg" /></div>;

  if (!isConnected) {
    return (
      <div className="flex-1 flex flex-col items-center justify-center gap-4 px-5">
        <p className="text-text-primary font-sans">Connect wallet to view this session</p>
        <Button onClick={login}>Connect Wallet</Button>
      </div>
    );
  }

  if (session.loading) return <div className="flex-1 flex items-center justify-center"><Spinner size="lg" /></div>;
    ...
    return (
      <div className="flex-1 flex flex-col items-center justify-center gap-6 px-5 pb-24">
        ...
        <div className="text-center">
          <h2 className="font-display font-bold text-xl text-text-primary mb-2">Waiting for Opponent</h2>
          <p className="text-text-secondary text-sm font-sans">Finding an opponent for you now.</p>
        </div>
        <Button variant="ghost" onClick={() => router.push('/pvp/lobby')}>Cancel</Button>
      </div>
    );
  }
  ...
  if (phase === 'submitted') {
    return (
      <div className="flex-1 flex flex-col items-center justify-center gap-4 px-5">
        <Spinner size="lg" />
        <p className="font-display font-bold text-xl text-text-primary">Waiting for opponent…</p>
        <p className="text-text-secondary text-sm font-sans">Your answers submitted. Hang tight.</p>
      </div>
    );
  }
  ...
  if (phase === 'done' && session.winner) {
    ...
    return (
      <div className="flex-1 flex flex-col items-center justify-center gap-6 px-5 pb-24">
        ...
        <Button onClick={() => router.push('/pvp/lobby')} size="lg" className="w-full max-w-xs">
          Play Again
        </Button>
        <Button variant="ghost" onClick={() => router.push('/')} size="sm">Home</Button>
      </div>
    );
  }
