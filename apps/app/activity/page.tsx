...
function formatCelo(wei: number): string {
  return (wei / 1e18).toFixed(6) + " CELO";
}

function formatAmount(amount: number, mode: GameMode): string {
  if (mode === "free" || amount === 0) return "—";
  const celo = (Math.abs(amount) / 1e18).toFixed(4);
  return amount > 0 ? `+${celo} CELO` : `-${celo} CELO`;
}

function formatSignedCelo(amount: number): string {
  const celo = (Math.abs(amount) / 1e18).toFixed(4);
  if (amount > 0) return `+${celo} CELO`;
  if (amount < 0) return `-${celo} CELO`;
  return "0 CELO";
}
...
export default function PlayerLogPage() {
  ...
  const handleWithdraw = useCallback(async () => {
    ...
    try {
          ...
          await sendTransaction(
            {
              ...
              data: encodeFunctionData({ abi: casualPoolAbi, functionName: "withdrawRoyalty", args: [] }),
              chainId: 42220,
              gasLimit,
            },
            {
              ...
              uiOptions: {
                showWalletUIs: true,
                description: "Withdraw CELO royalties to your wallet",
                buttonText: "Withdraw",
      ...
      clearPendingRoyalty();
      setWithdrawSuccess(`Escrow withdrawn. ${formatCelo(pendingRoyalty)} sent to your wallet.`);
      refreshCreatorStats();
    } catch (e) {
